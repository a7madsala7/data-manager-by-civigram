...
def autocaded():
    ...
    def saving():
        ...
        for file_name in os.listdir("C:\\Users\\" + user_folder_name + "\\Civigram"):
            if file_name.endswith('.txt'):
                os.remove(os.path.join("C:\\Users\\", user_folder_name, "Civigram", file_name))
    ...
    def get_file_paths_from_listbox():
        ...
        for h in selected_file_paths:
            ...
            def copy_content():
                points_text.config(state="normal")  # Enable editing
                points_text.tag_add(SEL, "1.0", END)  # Select all text
                points_text.event_generate("<<Copy>>")  # Copy selected text
                points_text.config(state=DISABLED)  # Disable editing

            # Create the copy button
            copy_button = Button(window, text="Copy", command=copy_content)
            copy_button.pack()
